...
const OrderBook: React.FC<OrderBookProps> = ({ baseDenom, quoteDenom }) => {
  ...
  useEffect(() => {
    // Auto-refresh every 10 seconds if needed
  ...
  return (
    <div className="p-4 border rounded shadow-lg bg-white dark:bg-gray-800">
      <h2 className="text-xl font-bold mb-4">Order Book: {baseDenom}/{quoteDenom}</h2>
      ...
      <div className="grid grid-cols-2 gap-4 mt-6">
        <div>
          ...
          <ul>
            {buyOrders.map(order => (
              <li key={order.id} className="flex justify-between text-green-600">
        ...
        <div>
          ...
          <ul>
            {sellOrders.map(order => (
              <li key={order.id} className="flex justify-between text-red-600">
                <span>{order.quantity} {baseDenom}</span>
                <span>@ {order.price} {quoteDenom}</span>
                <button
                  className="ml-2 text-red-500 hover:underline"
